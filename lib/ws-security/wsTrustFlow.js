...
function WSTrustFlow(opts) {
	...
	function requestSecurityToken (opts, cb) {
		...
		soapClient.send(opts.wstrustEndpoint, message, function(err, res) {
			if (err) {
				cb(Error.create('An error ocurred trying to obtain the token.', ops, err));
				return;
			}

			if (res.statusCode != 200) {
				cb(Error.http(res.statusCode, 'An error ocurred trying to obtain the token.', res.body, err));
				return;
			}
