...
class Auth {
  constructor(settings) {
    ...
    var authenticateUsingNTLM = function(options, cb) {
      ...
      get(authOptions, function(err, res) {
        if (err) {
          return cb(err);
        }
        ...
        var cookies = parse(res.headers["set-cookie"].join(";")),
          authToken = v4(),
          session = {
    ...
    this.Do = function(options, cb) {
      var responseXMLCB = function(err, resXml) {
          ...
          var token = xpath
              .select("//*[local-name()='EncryptedData']", resXml)
              .toString(),
            authToken = v4(),
