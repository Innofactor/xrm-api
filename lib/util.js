    ...
    */
    ...
    executeSoapPost = function (options, action, template, body, cb) {
        var timeCreated = new Date(),
            ...
            soapHeader = soapHeaderMessage
                ...
                .replace("{encryptedData}", options.encryptedData),
                    ...
                    if (location.length > 0) {
                        ...
                        https.get(opts, function (res) {
                            ...
                            res.on('end', function () {
                                ...
                                authenticationType = xpath.select("//*[local-name()='Authentication' and namespace-uri()='http://schemas.microsoft.com/xrm/2011/Contracts/Services']/text()", resXmlImport).toString();
                                authHost = xpath.select("//*[local-name()='Identifier']/text()", resXmlImport).toString();
                                authHost = urljs.parse(authHost).host;
                                path = "/adfs/services/trust/2005/usernamemixed";
                                var loginEndpoint = "https://" + settings.domain + settings.domainUrlSuffix,
                                ...
                                samlRequest = microsoftOnlineSaml
                                    .replace("{username}", username)
                                    .replace("{password}", password)
                                    .replace("{toMustUnderstand}", "https://" + authHost + path)
                                    .replace("{endpoint}", loginEndpoint),
    ...
    this.Authenticate = function (options, cb) {
        ...
        responseXMLCB = function (err, resXml) {
            ...
            var authData = xpath.select("//*[local-name()='EncryptedData']", resXml).toString();

            self.cacheAuth.set(options.username, authData);
        ...
        if (settings.authType === "microsoft_online") {
            ...
            authenticateUsingFederation(options, responseXMLCB);
